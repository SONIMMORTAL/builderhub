...
export const Logo = (props: React.ComponentProps<"svg">) => (
    <svg
        ...
        {...props}
    >
        {/* Simplified Logo for brevity if needed, or use full path from prompt. 
        Actually, let's use the layout grid icon as the logo to match BuilderHub 
        or stick to the provided logo. The provided logo is nice. I will copy it. */}
        <defs>
            <clipPath id="a">
...
export function AuthSignUp({ onBack, onSuccess }: AuthSignUpProps) {
    ...
    return (
        <div className="relative w-full h-full min-h-[500px] flex items-center justify-center overflow-hidden bg-dark-950/80 rounded-3xl">
            <Particles
            ...
            <div className="relative z-10 w-full max-w-sm px-4 py-8 flex flex-col items-center">
                <button
                    onClick={onBack}
                    disabled={!!loading}
                    className="absolute top-4 left-4 flex items-center gap-1 text-sm text-gray-400 hover:text-white transition-colors disabled:opacity-50"
                >
                ...
                <div className="space-y-6 w-full text-center">
                    ...
                    <div className="space-y-1">
                        ...
                        <p className="text-base text-gray-400">
                            Login or create your BuilderHub account.
                        </p>
                    </div>
                    ...
                    <div className="space-y-3">
                        <Button
                            ...
                            disabled={!!loading}
                        >
                            {!loading && <GoogleIcon className="w-5 h-5 mr-2" />}
                            Continue with Google
                        </Button>
                        <Button
                            className="w-full bg-dark-800 text-white hover:bg-dark-700 border border-white/10"
                            variant="secondary"
                            onClick={() => handleLogin('github')}
                            isLoading={loading === 'github'}
                            disabled={!!loading}
                        >
                            {!loading && <GithubIcon className="w-5 h-5 mr-2" />}
                            Continue with GitHub
                        </Button>
                    </div>
                    <p className="text-xs text-gray-500 mt-8">
                        By clicking continue, you agree to our{" "}
                        <a className="underline underline-offset-4 hover:text-accent-purple" href="#" onClick={(e) => e.preventDefault()}>
                            Terms of Service
                        </a>{" "}
                        and{" "}
                        <a className="underline underline-offset-4 hover:text-accent-purple" href="#" onClick={(e) => e.preventDefault()}>
